...
export async function fetchOgImage(url: string): Promise<string | undefined> {
  try {
    return await domainThrottler.throttle(url, () => fetchOgImageInternal(url));
  } catch {
    return undefined;
  }
}

async function fetchOgImageInternal(url: string): Promise<string | undefined> {
...
export async function fetchOgImages(
  ...
  const results: (string | undefined)[] = Array.from({ length: items.length });
  let index = 0;
